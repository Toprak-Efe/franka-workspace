...
public:
  ...
  void add(const T &data) {
    ...
    _producer_state.index =
        _shared_state.index.exchange(write_idx, std::memory_order_acq_rel);
    _shared_state.new_data_available.store(true, std::memory_order_release);
  }

  bool get(T &data) {
    bool is_new = _shared_state.new_data_available.exchange(
        false, std::memory_order_acq_rel);
    if (is_new) {
      size_t reader_idx = _consumer_state.index;
      _consumer_state.index =
          _shared_state.index.exchange(reader_idx, std::memory_order_acq_rel);
    }
